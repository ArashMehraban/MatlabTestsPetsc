function Jac = get_global_jac(Jac, global_idx_map, msh,num_quadr_pts_in_1d,sz_u_field, userdf,all_grad_u)
...
%               
     ...
     for i=1:num_elem
         ...
         for k=1:size(BI,2)
             ...
             %f = userdf(reshape(BI(:,k),sz_u_field,[])', reshape(DI(:,k),sz_u_field,[])', reshape(DI(:,k),sz_u_field,[])',xe);
             f = userdf(reshape(BI(:,k),sz_u_field,[])', reshape(DI(:,k),sz_u_field,[])', all_grad_u((i-1)*size(BI,1)+1:i*size(BI,1),:),xe);
             [r,c]=size(f);
         ...
         % global jacobian assembly (consistent tangent)
         temp=conn(i,:)';
         k=1:neldof;
         kk=temp(k);
